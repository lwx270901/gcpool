...
struct VirDevPtr {
    VirDevPtr(CUdeviceptr addr_in, size_t allocSize_in, int device_id = -1): allocSize(allocSize_in), mapped(false), device_id(device_id), status(CUDA_SUCCESS), released(false) {
        ...
        if(status != CUDA_SUCCESS || (request_ptr != 0ULL && device_ptr != request_ptr)) {
            GCPOOL_INFO(" request_ptr: %p, device_ptr: %p", (void*)request_ptr, (void*)device_ptr);
...
struct VmmSegment {
    ...
    void allocate_phy_blocks(size_t blocks, size_t block_size_in, int device_id_in) {
        ...
        for(size_t i=0; i<blocks; i++)
        {
            ...
            if(phy_block->status != CUDA_SUCCESS) {
                ...
                GCPOOL_INFO(" allocate memory handle for %luth phy_block failed, current memory info: device_total: %luMB, device_free: %luMB, request size: %luMB, already allocate: %luMB",
    ...
    void* mapVirAddr() {
        ...
        do
        {
            ...
            if(vir_dev_ptr->status != CUDA_SUCCESS || !vir_dev_ptr->virAddr) {
                GCPOOL_INFO(" reserve memory of size %fMB failed", segment_size/(1024.f*1024.f));
                ...
                for(size_t j = 0; j < phy_blocks.size(); j++) {
                    ...
                    if(vir_block->status != CUDA_SUCCESS) {
                        ...
                        vir_blocks.clear();
                        cudaGetLastError();
                        GCPOOL_INFO(" map memory %p of size %fMB for the %luth phy_block failed", vir_block->block_ptr, granul_size/(1024.f*1024.f), j);
                        break;
                    } else {
    ...
    std::shared_ptr<VmmSegment> split(size_t keep_size) {
        if(keep_size%granul_size) {
            GCPOOL_INFO(" keep_size %fMB is not multiple of granul_size %fMB", keep_size/(1024.f*1024.f), granul_size/(1024.f*1024.f));
            gtrace();
            exit(-1);
        }
        ...
        if(keep_blocks >= vir_blocks.size()) {
            GCPOOL_INFO(" keep_blocks %lu is larger than remaing blocks %lu", keep_blocks, vir_blocks.size());
            gtrace();
            exit(-1);
        }
    ...
    bool remerge(VmmSegment& segment) {
        if( segment.segment_ptr ==  (void*) ( (char*)this->segment_ptr + this->phy_blocks.size()*granul_size) ) {
           ...
           this->segment_ptr = segment.segment_ptr;
            ...
            GCPOOL_INFO(" segment of ptr %p size %fMB is not head or tail of segment ptr %p size %fMB", 
